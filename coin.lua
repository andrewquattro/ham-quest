...
function Coin.new(x,y)
    ...
    instance.x = x
    instance.y = y
    instance.img = love.graphics.newImage("assets/ham.png")
...
function Coin:remove()
    for i,instance in ipairs(ActiveCoins) do
        if instance == self then
            if Player.alive and not Player.beatGame then
...
function Coin:spin(dt)
    self.scaleX = math.sin(love.timer.getTime() + self.randomTimeOffset)
end

function Coin:draw()
    love.graphics.draw(self.img, self.x, self.y, 0,self.scaleX * 0.5,0.5, self.width / 2, self.height / 2)
end
